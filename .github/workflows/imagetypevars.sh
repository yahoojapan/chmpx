...
#
RUNNER_INSTALL_PACKAGES="git autoconf autotools-dev gcc g++ make gdb dh-make fakeroot dpkg-dev devscripts libtool pkg-config procps libyaml-dev k2hash-dev gnutls-dev dh-systemd"
...
if [ -z "${CI_DOCKER_IMAGE_OSTYPE}" ]; then
	...
	PKG_INSTALL_LIST_BUILDER="git build-base openssl-dev libtool automake autoconf pkgconf yaml-dev procps"
	PKG_INSTALL_LIST_BIN="libstdc++ yaml"
